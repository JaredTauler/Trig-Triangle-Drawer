...
class Triangle
{
    ...
    {
        this.points = [
            new Point(CursorPosition, this),
            new Point([CursorPosition-50,CursorPosition-100], this),
            new Point([CursorPosition+50,CursorPosition-100], this)
        ]
        this.right = false
    }
    ...
    // We need to leave ADJ and HYP where they lay, but must have HYP first.
    arrange()
    {
        ...
        for (let i of this.points) {
            if (biggest == null || i.side.length > biggest.side.length) {
                biggest = i
            }
        }
        // Remove Hyp and place at front of array
        this.points.splice(biggest.i, 1)
        console.log(this.points)
        this.points.unshift(biggest)
    }
}
...
class Point
{
    ...
    set position(xy)
    {
        // if (this.last)
        // {
        //     this.last = structuredClone(this)
        // }
        [this.x, this.y] = xy
    }
    ...
    get i()
    {
        return this.triangle.points.findIndex((p) => Object.is(p, this))
    }
}
...
var Closest = {
    ...
    // get pointi () {
    //     return this.point.i(this.triangle)
    // }
}
...
function SetClosest()
{
    CurrentTriangle = Closest.triangle
    let tri = Closest.triangle
    if (tri.right) // Oblique
    {
        // tri.arrange()
        let i = Closest.point.i
        ...
        } else // ADJ / OPP
        {
            ...
            let dir = 1
            sis = ((i === 1) ? 2 : 1)
            dir = ((i === 1) ? -1 : 1) * Radians(90) // + or - 90 degrees depending on rather selected point is opp or adj

            let a = tri.points[i]
            let b = tri.points[0]
            let c = tri.points[sis]

            Closest.triangle.points[sis].position = AroundPoint([a, b, c], dir)
...
function AroundPoint([a, b, c], dir)
{
    var angle = Math.atan2(a.y - b.y, a.x - b.x) // get angle of starting point
    angle += dir
